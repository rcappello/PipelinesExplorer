...
{
    public override CommandConfiguration CommandConfiguration => new("Pipelines Explorer: Select Microsoft tenant")
    {
    ...
    public override async Task ExecuteCommandAsync(IClientContext context, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (tenants.Count == 0)
            {
                await this.Extensibility.Shell().ShowPromptAsync(
                    "No tenants are available for the signed-in account.",
                    PromptOptions.OK,
        ...
        catch (Exception ex)
        {
            ...
            await this.Extensibility.Shell().ShowPromptAsync(
                $"Select tenant failed: {ex.Message}",
                PromptOptions.OK,
                cancellationToken).ConfigureAwait(false);
        }
    }
